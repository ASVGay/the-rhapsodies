...
export default function TopNavigation() {
    ...
    return (<div className={"hidden lg:flex bg-white h-24 justify-between items-center px-2"}>
            ...
            {NavigationItems.map((item, index): React.ReactNode => {
                ...
                return <Link key={item.path} href={item.path}
                    ...
                    <div className={"relative group "}>
                        <span>{item.text}</span>
                        <span
                            className="absolute -bottom-1 left-0 w-0 transition-all h-0.5 bg-moon-300 group-hover:w-full"></span>
                    </div>
                </Link>
            })}
        </div>
