...
const SuggestionCard = ({ suggestion }: SuggestionCardProps) => {
  ...
  const progressionBarWidth =
    (rolesFilled() / suggestion.roles.length) * 100 + "%"
    ...
    <div className={"w-[22rem] rounded-md bg-neutral-50 drop-shadow-lg"}>
      <div className={"flex items-start p-3"}>
        ...
        <span className={"pl-3"}>
          ...
          <p
            className={
              "line-clamp-3 h-12 text-sm font-medium leading-4 text-gray-400"
            }
          >
        ...
        <div className={"flex items-center justify-between"}>
          <div className={"h-2 w-full rounded-md bg-green-200"}>
            <div
              ...
              style={{ width: progressionBarWidth }}
            />
          </div>
          <p className={"ml-4 text-sm font-light text-gray-400"}>
            {progressionFraction}
          </p>
        </div>
